...
int setupRaSocket(int fd, int ifIndex) {
    ...
    if (setsockopt(fd, IPPROTO_ICMPV6, ICMP6_FILTER, &rs_only, len) != 0) {
        RLOGE("setsockopt ICMP6_FILTER error:%s", strerror(errno));
        return ERROR;
    }
    ...
    if (setsockopt(fd, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, &hops, len) != 0) {
        RLOGE("setsockopt IPV6_MULTICAST_HOPS error:%s", strerror(errno));
        return ERROR;
    }
    if (setsockopt(fd, IPPROTO_IPV6, IPV6_UNICAST_HOPS, &hops, len) != 0) {
        RLOGE("setsockopt IPV6_UNICAST_HOPS error:%s", strerror(errno));
        return ERROR;
    }
    ...
    if (setsockopt(fd, IPPROTO_IPV6, IPV6_MULTICAST_LOOP, &off, len) != 0) {
        RLOGE("setsockopt IPV6_MULTICAST_LOOP error:%s", strerror(errno));
        return ERROR;
    }
    ...
    if (setsockopt(fd, IPPROTO_IPV6, IPV6_MULTICAST_IF, &ifIndex, len) != 0) {
        RLOGE("setsockopt IPV6_MULTICAST_IF error:%s", strerror(errno));
        return ERROR;
    }
    ...
    if (bind(fd, (struct sockaddr*) &sin6, len) != 0) {
        RLOGE("bind(IN6ADDR_ANY):%s", strerror(errno));
        return ERROR;
    }
    ...
    if (setsockopt(fd, IPPROTO_IPV6, IPV6_JOIN_GROUP, &all_rtrs, len) != 0) {
        RLOGE("setsockopt (IPV6_JOIN_GROUP):%s", strerror(errno));
        return ERROR;
    }
...
int createRawSocket(const char* iface) {
    ...
    if (rawSocket < 0) {
        RLOGE("rawSocket error:%s", strerror(errno));
        return ERROR;
    }
    ...
    if (setsockopt(rawSocket, SOL_SOCKET, SO_SNDTIMEO, &timeout, len) < 0) {
        RLOGE("setsockopt SO_SNDTIMEO error:%s", strerror(errno));
        return ERROR;
    }
    if (setsockopt(rawSocket, SOL_SOCKET, SO_BINDTODEVICE, iface, strlen(iface) + 1) < 0) {
        RLOGE("setsockopt SO_BINDTODEVICE error:%s", strerror(errno));
        return ERROR;
    }

    unsigned int index = if_nametoindex(iface);
    if (setupRaSocket(rawSocket, index) < 0) {
        RLOGE("setupRaSocket error:%s", strerror(errno));
        return ERROR;
    }
    RLOGI("createRawSocket done");
    return rawSocket;
}
...
void getHwAddr(const char* iface, byte* mac) {
    ...
    if ((ioctl(sockId, SIOCGIFHWADDR, &ifr_mac)) < 0) {
        close(sockId);
        RLOGE("SIOCGIFHWADDR error:%s", strerror(errno));
        return;
    }
    ...
    close(sockId);
    RLOGD("getHwAddr:%s %s", iface, mac_addr);
}
...
int sendRs(int socketFd, char* iface) {
    ...
    inet_pton(AF_INET6, IN6ADDR_ALL_ROUTERS, &destAddr6.sin6_addr);
    addrSize = sizeof(destAddr6);

    numBytes = sendto(socketFd, raData, dataSize, 0,
                                (const struct sockaddr*) &destAddr6, addrSize);
    if (numBytes < 0) {
        RLOGE("Send failure with %s", strerror(errno));
        return ERROR;
    } else {
        RLOGD("Send IPv6 RS: %d:%zd", dataSize, numBytes);
    }
...
int triggerIpv6Rs(char* iface) {
    ...
    if (sockFd < 0) {
        RLOGE("Failed to createRawSocket with %s", iface);
        return ERROR;
    }
    sendRs(sockFd, iface);
    return SUCCESS;
